...
7. Для продовой сборки: `bun run build` упакует серверную часть Bun-бандлером в `dist/server.js`, после чего запускайте `bun start`. Фронтенд по-прежнему собирается через Vite (`bun run build:admin`).

⚠️ Если вы столкнётесь с ошибкой вида:

```
error: Could not resolve: ".prisma/client/default"
```

то это означает, что Prisma Client не был сгенерирован или bundler пытается резолвить внутренние файлы клиента во время сборки. Рекомендуемые шаги:

- Убедитесь, что `prisma generate` выполнен перед сборкой. В этом репозитории `postinstall` теперь запускает `bunx prisma generate`.
- Bun bundлер не должен включать `node_modules` — это делается через `bunfig.toml: packages = "external"` и параметр `external = ["@prisma/client"]`. Это оставляет клиент в `node_modules` и гарантирует правильный резолв `.prisma` путей во время выполнения.

Эти изменения уже применены в `package.json` и `bunfig.toml` — при деплое убедитесь, что Heroku (или CI) запускает `bun install` и/или `bun run build` в правильной очередности.
...
- **Backend:** Bun 1+, Telegraf, Bun.serve, Prisma, PostgreSQL.
  - Примечание: проект обновлён до Prisma v7; генератор клиента использует `provider = "prisma-client"`. Также включена preview-фича `clientExtensions` — это добавляет удобные методы доступа к данным сессии. Подробнее в `prisma/schema.prisma`.
