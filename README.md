...
- персонализация рекомендаций через rule-based scoring поверх каталога секций;
- единая инфраструктура Fastify обслуживает и бота, и REST API для SPA;
- Prisma + PostgreSQL сохраняют заполненные анкеты и статистику;
- статические данные (`sections.ts`) дополняются изображениями и прогнозами результатов.
...
- интерактивный wizard с шагами: возраст → пол → подготовка → формат → цели;
...
> 💡 Замените placeholder на реальный GIF или скриншот диалога, разместив его в `docs/media`.
...
3. Примените схему данных: `npx prisma migrate deploy`.
4. Запустите сервер и бота: `npm run dev` (использует `tsx`).
5. Для SPA админки — отдельный `npm run dev:admin` (Vite).
