...
⚠️ Если во время сборки вы видите ошибку:

```
error: Could not resolve: ".prisma/client/default"
```

— это означает, что Prisma Client не сгенерирован или bundler пытался разрешить `.prisma` файлы при сборке. Решение:

- Убедитесь, что `postinstall` запускает `bunx prisma generate` (в репозитории это включено в `package.json`).
- Примечание: при переходе на Prisma v7 URL для `datasource` нужно перемещать в `prisma.config.ts`; схема `schema.prisma` не должна содержать `url` — это сделано в репозитории и `prisma.config.ts` автоматически использует `DATABASE_URL`.
- В `bunfig.toml` проект настроен с `packages = "external"`, так что Bun не попытается инлайнить `node_modules` во время сборки; вместо этого `@prisma/client` разрешается при старте приложения из `node_modules`, где есть сгенерированные `.prisma` файлы.
