...
> ⚠️ Важно по поводу Prisma v7 и Heroku/Bun
>
> Если при выполнении seed-скрипта (`prisma db seed`) возникает ошибка вида "Cannot find module './cjs/index.cjs'" (особенно при запуске `tsx prisma/seed.ts` под Bun/Heroku), это связано с особенностями разрешения точек входа пакетов в некоторых окружениях. В этом репозитории seed-скрипт специально импортирует адаптер Postgres из ESM-dist-энрипойнта (`@prisma/adapter-pg/dist/index.js`) чтобы обойти проблему пакета пытающегося разрешить CJS-энтри и падения сборки.

> Если вы используете другой runtime или ещё сталкиваетесь с проблемой, убедитесь, что в вашем окружении установлена подходящая версия `@prisma/adapter-pg` и что `prisma db seed` выполняется через совместимый интерпретатор (например `bunx prisma db seed` / `node` / `tsx` в соответствии с вашей конфигурацией).
